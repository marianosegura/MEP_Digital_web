...
export default function AssignmentsItem(props) {
  const type = useSelector((state) => state.userInfo.type);
  const courseId = useSelector(state => state.coursesInfo.selectCourse.id)
  const handleClick = (e) => {
    e.preventDefault()
        var direction = "https://desolate-everglades-59280.herokuapp.com/api/courses/" + courseId + "/assignments/" + props.id //id = _id
        var myInit = {
            method: "DELETE",
